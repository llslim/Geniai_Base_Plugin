...
if ($action === 'upload' && confirm_sesskey()) {
    if (!empty($_FILES['jsonfile']['tmp_name'])) {
        ...
        if (json_last_error() === JSON_ERROR_NONE && !empty($data['scenario_id']) && !empty($data['persona']['name'])) {
            ...
            $record = $DB->get_record('local_geniai_custom_scenarios', ['scenariocode' => $scenariocode]);
            if ($record) {
                $record->name = $name;
                ...
                $DB->update_record('local_geniai_custom_scenarios', $record);
                ...
                $record->timemodified = time();
                $DB->insert_record('local_geniai_custom_scenarios', $record);
...
if ($action === 'delete' && confirm_sesskey()) {
    ...
    $record = $DB->get_record('local_geniai_custom_scenarios', ['scenariocode' => $deletecode]);
    if ($record) {
        $DB->delete_records('local_geniai_custom_scenarios', ['id' => $record->id]);
...
$customscenarios = $DB->get_records('local_geniai_custom_scenarios', null, 'name ASC');

$PAGE->set_context(context_system::instance());
$PAGE->set_url("/local/geniai/scenario_builder.php");
$PAGE->set_title(get_string("modulename", "local_geniai") . " - Scenario Builder & Management");
$PAGE->set_heading(get_string("modulename", "local_geniai") . " - Scenario Builder & Management");
