...
use Exception;
use local_aacuracore\local\editor\editor_tiny;
...
class page_create {
    ...
    public function create_page() {
        ...
        $h5pjs = $curl->post("https://app.ottflix.com.br/api/v1/H5pjs", [
            ...
            "frequency_penalty" => get_config("local_aacuracore", "frequency_penalty"),
            "presence_penalty" => get_config("local_aacuracore", "presence_penalty"),
            "apikey" => get_config("local_aacuracore", "apikey"),
            "module" => "create",
        ]);

        $basecolor = get_config("local_aacuracore", "base_color");
        $types = types::get_types($this->h5p->contextid);
        echo $OUTPUT->render_from_template("local_aacuracore/h5p-create", [
            "h5p" => $this->h5p,
            "user" => $USER,
            "user_lang" => isset($SESSION->lang) ? $SESSION->lang : $USER->lang,
            "h5pjs" => $h5pjs,
    ...
    public function edit() {
        ...
        $types = types::get_types($this->h5p->contextid);
        echo $OUTPUT->render_from_template("local_aacuracore/h5p-edit", [
            "h5p" => $this->h5p,
            ...
            "pages" => array_values($DB->get_records("local_aacuracore_h5ppages", ["h5pid" => $this->h5p->id])),
    ...
    public function save() {
        ...
        if (isset($config["baseColor"])) {
            set_config("base_color", $config["baseColor"], "local_aacuracore");
        }
        ...
        if ($this->h5p->id) {
            ...
            $DB->update_record("local_aacuracore_h5p", $this->h5p);
        } else {
            ...
            $this->h5p->id = $DB->insert_record("local_aacuracore_h5p", $this->h5p);
        }

        $h5ppages = $DB->get_records("local_aacuracore_h5ppages", ["h5pid" => $this->h5p->id]);
        $pages = $this->params_array("pages", PARAM_RAW, false);
        foreach ($pages as $page) {
            if ($h5ppage = $DB->get_record("local_aacuracore_h5ppages", ["h5pid" => $this->h5p->id, "type" => $page["type"]])) {
                $h5ppage->title = $page["title"];
                $h5ppage->type = $page["type"];
                $h5ppage->data = json_encode($page, JSON_PRETTY_PRINT);
                $DB->update_record("local_aacuracore_h5ppages", $h5ppage);
                ...
                $DB->insert_record("local_aacuracore_h5ppages", $h5ppage);
            }
        }

        foreach ($h5ppages as $h5ppage) {
            if (isset($h5ppage->id)) {
                $DB->delete_records("local_aacuracore_h5ppages", ["id" => $h5ppage->id]);
            }
        }
    }
    ...
    public function delete() {
        ...
        $DB->delete_records("local_aacuracore_h5p", ["id" => $this->h5p->id]);
        $DB->delete_records("local_aacuracore_h5ppages", ["h5pid" => $this->h5p->id]);
    }
    ...
    public function send_contentbank($contentbankid = null) {
        ...
        if ($this->h5p->contentbanktid) {
            ...
            $DB->execute("UPDATE {local_aacuracore_h5p} SET contentbanktid = '{$content->get_id()}' WHERE id = {$this->h5p->id}");
        }
     ...
     * Function set_h5p
     *
     * @param local_aacuracore_h5p $h5p
     */
